...
library TTGRegistrarReader {
    ...
    /// @notice The parameter name in TTG that required interval to update collateral.
    bytes32 internal constant UPDATE_COLLATERAL_INTERVAL = "update_collateral_interval";

    /// @notice The parameter name in TTG that enforces the max number of allowed collateral update per interval.
    bytes32 internal constant MAX_UPDATE_COLLATERAL_PER_INTERVAL = "update_collateral_per_interval";
    ...
    /// @notice Gets the max number of allowed collateral update per interval.
    function getMaxUpdateCollateralPerInterval(address registrar_) internal view returns (uint256) {
        return uint256(_get(registrar_, MAX_UPDATE_COLLATERAL_PER_INTERVAL));
    }
