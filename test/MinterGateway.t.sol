...
contract MinterGatewayTests is TestUtils {
    ...
    function testFuzz_updateCollateral(
        uint256 collateral_,
        uint256 threshold_,
        uint256 numberOfSignatures_,
        uint256 salt_
    ) external {
        salt_ /= 2; // Divide by 2 to avoid overflow.
        ...
        uint256[] memory pks_ = new uint256[](numberOfSignatures_);
        address[] memory validators_ = new address[](numberOfSignatures_);
        bytes[] memory signatures_ = new bytes[](numberOfSignatures_);

        uint40 mintTimestamp_;

        for (uint256 i; i < numberOfSignatures_; ++i) {
            if (i == 0) {
                (validators_[0], pks_[0]) = makeAddrAndKey(string(abi.encode("validator", i)));
            } else {
                // Next validator's address must be bigger to preserve right order of signers
                while (validators_[i] <= validators_[i - 1]) {
                    (validators_[i], pks_[i]) = makeAddrAndKey(string(abi.encode("validator", i, salt_++)));
                }
            }
            ...
            timestamps_[i] = bound(
                salt_++,
                vm.getBlockTimestamp() - _updateCollateralInterval / 2,
                vm.getBlockTimestamp()
            );

            if (i == 0 || timestamps_[i] < mintTimestamp_) {
                mintTimestamp_ = uint40(timestamps_[i]);
            }
        ...
        vm.expectEmit();
        emit IMinterGateway.CollateralUpdated(_minter1, uint240(collateral_), 0, bytes32(0), mintTimestamp_);
        ...
        assertEq(_minterGateway.collateralUpdateTimestampOf(_minter1), mintTimestamp_);
        assertEq(_minterGateway.collateralExpiryTimestampOf(_minter1), mintTimestamp_ + _updateCollateralInterval);
        assertEq(_minterGateway.collateralPenaltyDeadlineOf(_minter1), mintTimestamp_ + _updateCollateralInterval);
