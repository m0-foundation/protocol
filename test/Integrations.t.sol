...
contract IntegrationTests is Test {
    ...
    function setUp() external {
        ...
        (address protocol, address earnerRateModel, address minterRateModel) = _deploy.deploy(
            _deployer,
            0,
            address(_registrar)
        );

        _earnerRateModel = IEarnerRateModel(earnerRateModel);
        _minterRateModel = IMinterRateModel(minterRateModel);
        _protocol = IProtocol(protocol);
        ...
        _registrar.updateConfig(SPOGRegistrarReader.MINTER_RATE_MODEL, address(_minterRateModel));
